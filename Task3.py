...
class NotDigit(Exception):
    def __init__(self, digit):
        self.digit = digit
    def __str__(self):
        return f"Этот '{self.digit}' элемент - не число и соответственно не будет добавлен  в список!"
    def __repr__(self):
        return "Собственное исключение для проверки совместимости элемента с числовым типом"
def isNumeric(numeric, my_list):
    if numeric.isnumeric():
        my_list.append(int(numeric))
    else: raise NotDigit(numeric)

my_list = []
while True:
    try:
        isNumeric( input("Введите ваше число >> "), my_list )
    except NotDigit:
        print("Вы ввели не числовой элемент, пожалуйста попробуйте еще раз!")
    finally: print(my_list)
